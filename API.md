...
代理 [Freesound APIv2 Search][freesound-search]。该 Freesound 资源不是 OAuth2 required，但 Freesound API 仍要求 token authentication；需要在服务端配置 `FREESOUND_API_KEY`，Voxout 会用 `Authorization: Token ...` 调用上游。响应保持 Freesound sound list 结构：`{ count, next, previous, results }`。

| 实际传参 | Freesound 字段 | 默认/限制 |
|---|---|---|
| `query` 或 `q` | `query` | 可空，空查询返回 Freesound 全量搜索结果 |
| `filter` | `filter` | Freesound Solr filter syntax |
...
| `fields` | `fields` | 默认 `id,name,tags,username,license,url,previews,duration,type` |
| `page` | `page` | 默认 `1`，最小 `1` |
