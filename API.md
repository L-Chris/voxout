...
代理 [Freesound APIv2 Search][freesound-search]。该 Freesound 资源不是 OAuth2 required；如配置 `FREESOUND_API_KEY`，Voxout 会用 `Authorization: Token ...` 调用上游，否则不带认证头调用。响应保持 Freesound sound list 结构：`{ count, next, previous, results }`。
